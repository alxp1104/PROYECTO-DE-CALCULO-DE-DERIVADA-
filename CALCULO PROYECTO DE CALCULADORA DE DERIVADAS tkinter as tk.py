...
import numpy as np
from sympy import symbols, diff, parse_expr, sympify, solve, Eq, Derivative
...
class GraphManager:
    ...
    def plot_implicit(self, x_vals, y_vals):
        self.ax.plot(x_vals, y_vals, color='#33B5FF', linewidth=2)
        self.canvas.draw()
...
class DerivativeCalculator:
    def __init__(self, root):
        ...
        self.is_implicit_var = tk.BooleanVar(value=False)
        self.derivative_var = tk.StringVar(value="x")  # Variable para derivar (x o y)
        self.current_theme = 'dark'
        self.history = []
    ...
    def configure_dark_theme(self):
        ...
        style.configure('TLabelframe', background='#121212', foreground='#33B5FF')
    ...
    def create_widgets(self):
        ...
        # Checkbox para 3D e implícitas
        options_frame = ttk.Frame(input_frame)
        options_frame.pack(fill=tk.X, pady=5)
        
        ttk.Checkbutton(options_frame, text="Gráfica 3D", 
                       variable=self.is_3d_var, command=self.toggle_3d).pack(side=tk.LEFT, padx=5)
        
        ttk.Checkbutton(options_frame, text="Derivada Implícita", 
                       variable=self.is_implicit_var, command=self.toggle_implicit).pack(side=tk.LEFT, padx=5)
        
        # Variable para derivar (x o y)
        ttk.Label(options_frame, text="Derivar respecto a:").pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(options_frame, text="x", variable=self.derivative_var, value="x").pack(side=tk.LEFT)
        ttk.Radiobutton(options_frame, text="y", variable=self.derivative_var, value="y").pack(side=tk.LEFT)
    ...
    def toggle_3d(self):
        if self.is_3d_var.get():
            self.is_implicit_var.set(False)
    ...
    def toggle_implicit(self):
        if self.is_implicit_var.get():
            self.is_3d_var.set(False)
            self.graph_manager.init_implicit_plot()
        else:
            self.graph_manager.init_2d_plot()
    ...
    def validate_input(self):
        ...
        try:
            # Verificar si es una ecuación implícita
            if "=" in expr:
                lhs, rhs = expr.split("=", 1)
                parse_expr(lhs.replace("^", "**")) - parse_expr(rhs.replace("^", "**"))
            else:
    ...
    def calculate_derivative(self):
        try:
            ...
            derivative_var = self.derivative_var.get()
            
            expr_str = expr_str.replace("^", "**")
            is_3d = self.is_3d_var.get()
            is_implicit = self.is_implicit_var.get()
            
            # Manejar derivadas implícitas
            if is_implicit or "=" in expr_str:
                self.calculate_implicit_derivative(expr_str, order, derivative_var)
                return
            ...
            # Determinar respecto a qué variable derivar
            deriv_var = symbols(derivative_var)
            
            derivada = expr
            for i in range(order):
                derivada = diff(derivada, deriv_var)
    ...
    def calculate_implicit_derivative(self, expr_str, order, derivative_var):
        try:
            # Separar la ecuación en lhs y rhs
            if "=" in expr_str:
                ...
                # Asumir que es una expresión implícita f(x,y)=0
                equation = Eq(parse_expr(expr_str.strip()), 0)
            ...
            if order == 1:
                # dy/dx = - (∂f/∂x) / (∂f/∂y)
                df_dx = diff(equation.lhs - equation.rhs, x)
                df_dy = diff(equation.lhs - equation.rhs, y)
                
                if derivative_var == "x":
                    # Queremos dx/dy
                    derivada = -df_dy / df_dx
                else:
                    # Queremos dy/dx
                    derivada = -df_dx / df_dy
                
                derivada = derivada.simplify()
            else:
                # Para derivadas de orden superior, usar diferenciación implícita iterativa
                derivada = None
                current_deriv = Derivative(y, x) if derivative_var == "y" else Derivative(x, y)
                
                for i in range(order):
                    # Resolver para la derivada actual
                    current_expr = equation.lhs - equation.rhs
                    current_expr = current_expr.subs(y, y(x) if derivative_var == "y" else current_expr.subs(x, x(y)))
                    current_diff = diff(current_expr, deriv_var if i == 0 else other_var)
                    
                    # Resolver para la derivada actual
                    if i == 0:
                        derivada = solve(current_diff, current_deriv)[0]
                    else:
                        derivada = diff(derivada, other_var)
                        derivada = derivada.simplify()
            
            self.original_expr.config(text=str(equation).replace("**", "^"))
            self.derivative_expr.config(text=f"d{derivative_var}/d{'x' if derivative_var == 'y' else 'y'} = {str(derivada).replace('**', '^')}")
            
            # Graficar la función implícita
            self.plot_implicit_function(equation, derivative_var)
            
            self.current_expr = equation
            self.current_derivative = derivada
            self.add_to_history(str(equation))
            
        except Exception as e:
            messagebox.showerror("Error", f"Error al calcular derivada implícita: {e}")
    
    def plot_implicit_function(self, equation, derivative_var):
        try:
            ...
            x, y = symbols('x y')
            f = lambdify_safe((x, y), equation.lhs - equation.rhs)
            ...
            # Encontrar los contornos donde f(x,y) = 0
            self.graph_manager.ax.clear()
            cs = self.graph_manager.ax.contour(X, Y, Z, levels=[0], colors='#33B5FF')
            
            try:
                # Versión moderna de Matplotlib (3.3.0+)
                for collection in cs.collections:
                    for path in collection.get_paths():
                        ...
                        if len(vertices) > 1:
                            ...
                            self.graph_manager.ax.plot(x_contour, y_contour, color='#33B5FF', linewidth=2)
            except AttributeError:
                try:
                    # Versión antigua de Matplotlib
                    ...
                    messagebox.showerror("Error", f"No se pudo graficar la función implícita: {e}")
                    return
            
            self.graph_manager.ax.set_xlabel('x', color='white')
            self.graph_manager.ax.set_ylabel('y', color='white')
            
            title = f"Función Implícita: {str(equation).replace('**', '^')}"
            self.graph_manager.ax.set_title(title, color='white')
    ...
    def plot_functions(self, expr, derivada=None):
        try:
            ...
            if self.is_implicit_var.get() or (isinstance(expr, Eq) and "=" in str(expr)):
                self.plot_implicit_function(expr, self.derivative_var.get())
                return
    ...
    def find_extrema(self):
        try:
            ...
            # Manejar ecuaciones implícitas de manera diferente
            if isinstance(self.current_expr, Eq):
                messagebox.showinfo("Información", "La búsqueda de extremos no está disponible para ecuaciones implícitas.")
                return
    ...
    def use_example(self, example):
        ...
        # Detectar si es una ecuación implícita
        if "=" in example:
            self.is_implicit_var.set(True)
            self.is_3d_var.set(False)
            self.graph_manager.init_implicit_plot()
    ...
    def show_help(self):
        ...
        1. Ingrese su función en el campo de texto.
           - Para derivadas implícitas, use ecuaciones (ej: x^2 + y^2 = 1)
           - Para funciones 3D, active la opción correspondiente
        2. Seleccione respecto a qué variable derivar (x o y)
        3. Seleccione el orden de derivación.
        4. Haga clic en 'Calcular Derivada'.
